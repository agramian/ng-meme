angular.module('memeFactory', ['imageFromUrlFactory'])
.factory("MemeAPI", function($timeout, $http, ImageFromUrlAPI) {
	var MemeAPI = function() {
		...
		this.imageFromUrlAPI = new ImageFromUrlAPI(); 		
	};
	...
	MemeAPI.prototype.loadImage = function(image) {	
		...
		if (image.indexOf(".jpg") > -1
			...
			// if url		
				...
				var self = this;
				// eventually want to figure out angularjs
				// CORS issues and use this instead of the backup
				this.imageFromUrlAPI.getRawImageDataFromUrl(
					image,
					function(rawImageData){
						self.drawImageFromRawData(rawImageData);
					},
					function(rawImageData){
						self.imageFromUrlAPI.getRawImageDataFromUrlFromServer(
							image,
							function(rawImageData){
								self.drawImageFromRawData(rawImageData);
							},
	...
	MemeAPI.prototype.getMemeImageData = function(width, height, cutFromCanvas) {
		...
		var imageData = this.canvas.toDataURL("image/png").split(',')[1];
